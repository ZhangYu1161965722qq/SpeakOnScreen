...
fun PowerKeyScreen(
    ...
    var isServiceRunning by remember { mutableStateOf(ServiceHelper.isRunning(context, PowerListenerService::class.java)) }
    ...
    LaunchedEffect(Unit) {
        while (isActive) {
            // 每秒读取服务状态，同步UI
            isServiceRunning = ServiceHelper.isRunning(context, PowerListenerService::class.java)
