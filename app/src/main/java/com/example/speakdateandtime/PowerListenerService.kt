...
class PowerListenerService : Service(), TextToSpeech.OnInitListener {
    ...
    companion object {
        ...
        private const val DEBOUNCE_MS = 500L       // 防抖时间(毫秒)
    ...
    // ==================== 实例成员变量 ====================
    private var needSpeakTask = false   // 标记「现在有一条亮屏播报任务等着 TTS 初始化完成后补发」
    ...
    override fun onInit(status: Int) {
        ...
        if (isTtsReady) {
            ...
            if (needSpeakTask) {
                ...
                if (BuildConfig.DEBUG) Log.d(TAG, "TTS初始化完成，补发最新播报内容: $speakText")
    ...
    private fun isTtsReady(): Boolean {
        // ✅ 先检查标记，避免重复创建
        if (!this.isTtsReady || tts == null) return false

        return try {
            tts?.speak("", TextToSpeech.QUEUE_FLUSH, null, "check") == TextToSpeech.SUCCESS
        } catch (_: Exception) {
            false
        }
    }
    ...
    private fun speakCurrentTime() {
        ...
        if (isTtsReady()) {
            ...
            if (BuildConfig.DEBUG) Log.d(TAG, "播报内容: $speakText")
            // ==================== 执行语音播报 ====================
            ...
            override fun onReceive(context: Context?, intent: Intent?) {
                if (intent?.action == Intent.ACTION_SCREEN_ON) {
                    ...
                    if (now - lastSpeakTime >= DEBOUNCE_MS) {
                        ...
                        if (BuildConfig.DEBUG) Log.d(TAG, "防抖触发，忽略本次亮屏")
                    }
