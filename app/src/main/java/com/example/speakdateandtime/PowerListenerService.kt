...
class PowerListenerService : Service(), TextToSpeech.OnInitListener {
    ...
    override fun onInit(status: Int) {
        ...
        if (isTtsReady) {
            ...
            Log.d(TAG, "TTS 配置完成 - 语言: 中文, 语速: 0.6")
        }
    }
    ...
    private fun speakCurrentTime() {
        ...
        val speakText = timeText + batteryText

        // ==================== 调节音量 ====================
        // 保存当前音量并设置音量
        val percent : Double = if (period=="深夜") 0.3  else 0.8
    ...
    private fun getCurrentTimeText(): Pair<String, String> {
        ...
        val timeText = "现在时间：${period}${hour12}点${minute}分；${solarDate}；星期${weekDay}；${lunarDate}。"
