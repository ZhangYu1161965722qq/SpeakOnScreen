...
class PowerListenerService : Service(), TextToSpeech.OnInitListener {
    ...
    private fun recreateTts(): Boolean {
        ...
        // 3. 等待初始化完成（repeat是系统函数表示运行20次）
        repeat(20) {
            if (isTtsReady) return@repeat
            Thread.sleep(100)
        }
    ...
    private fun speakCurrentTime() {
        ...
        // ==================== 确保 TTS 可用 ====================
        // 如果 TTS 不可用，尝试重建；重建失败则放弃本次播报
        if (!isTtsReady() && !recreateTts()) {
            Log.e(TAG, "TTS 不可用，放弃播报")
            return
        }
